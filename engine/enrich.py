...
"""
Enrichment stage:
- For each candidate item, fetch details, credits, keywords, external IDs, and watch providers.
- If an item is missing a valid tmdb_id or details come back empty, fall back to tmdb.search_multi()
  using title (+ year when available) to resolve a better TMDB id, then enrich.
- Normalize TV 'networks' to [{"name": "..."}]; keep providers as slugs (summarize renders names).
- Write enriched list to items.enriched.json and append enrichment counters to diag.json.

This module is safe to call from engine.runner, or directly:
    python -m engine.enrich --in data/out/latest/items.discovered.json \
                             --out data/out/latest/items.enriched.json \
                             --run-dir data/out/latest
"""
...
def _int(n: str, d: int) -> int:
    try:
        return int(os.getenv(n, "") or d)
    except Exception:
        return d

REGION = os.getenv("REGION", "US")

# When to invoke search_multi() fallback
...
# Bound enrichment fetches (safety if huge pool)
...
def _choose_search_hit(mt: str, title: str, year: Optional[int], hits: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    """
    Score search_multi() hits with a simple blend:
      score = type_bonus + title_similarity - year_penalty
    """
    title = title or ""
    best = None
    best_score = -1e9
...
def _apply_details(mt: str, base: Dict[str, Any], details: Dict[str, Any]) -> None:
    if not details:
        return
    # merge but keep original title if present; fill year
    for k, v in details.items():
        if k in {"title","name"} and base.get(k):
            continue
        base[k] = v
...
def _enrich_one(item: Dict[str, Any], tel: Telemetry) -> Optional[Dict[str, Any]]:
    ...
    # Phase 1: If missing id OR details are empty, use search_multi to resolve
    details = {}
    if tmdb_id:
    ...
    need_search = False
    if SEARCH_MULTI_ON_MISSING_ID and not tmdb_id:
        need_search = True
    elif SEARCH_MULTI_ON_EMPTY_DETAILS and not details:
        need_search = True
    ...
    if need_search and title.strip():
        ...
        if best:
            ...
            # refresh mt/year/title from best
            mt = it["media_type"]
    ...
    # If we still don't have a tmdb_id or details, drop this item
    ...
    # Providers (slugs only; summarize renders names + filters to SUBS_INCLUDE)
    try:
    ...
    # Ensure media_type + year consistency
    it["media_type"] = mt
    if not it.get("year"):
...
def enrich_items(items: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], Telemetry]:
    ...
    # Limit to a reasonable batch for enrichment
    work = items[:ENRICH_SCORING_TOP_N] if ENRICH_SCORING_TOP_N > 0 else items
    for it in work:
        enriched = _enrich_one(it, tel)
        if enriched:
            out.append(enriched)
...
def _append_tel_to_diag(run_dir: Path, tel: Telemetry) -> None:
    ...
    # Namespaced enrichment counters (won't clash with pool/scoring)
    counts.update({
