...
def scrape_imdb_ratings(public_url: str, max_pages: int = 50) -> List[RatingItem]:
    """
    Best-effort HTML scrape of your public ratings page.
    If IMDb blocks the runner or ratings are private, returns [] or raises with a helpful hint.
    Prefer CSV when possible for reliability.
    """
    ...
    for _ in range(max_pages):
        ...
        rprint(f"[IMDb] page items={len(blocks)}")
        if not blocks:
            if start == 1:
                # Likely private list or bot protection—recommend CSV path
                raise ValueError("IMDb returned no items (ratings private or bot protection). Prefer CSV.")
            break
        ...
        for b in blocks:
            ...
            if sub:
                s = sub.get_text()
                if "TV Mini-Series" in s: typ = "tvMiniSeries"
                elif "TV Series" in s: typ = "tvSeries"
                elif "TV Movie" in s: typ = "tvMovie"
                elif "TV Special" in s: typ = "tvSpecial"
                elif "Video Game" in s: typ = "game"
            items.append(RatingItem(iid, title, year, typ, your))
