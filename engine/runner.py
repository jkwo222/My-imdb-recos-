...
def _env_from_os() -> Env:
    ...
    def _i(name: str, d: int) -> int:
        try: v=os.getenv(name,""); return int(v) if v else d
...
def _base_for_select(it: Dict[str, Any]) -> float:
    ...
    import math; return (v*2.0) + (math.log1p(p)*0.5)
...
def _enrich_providers(items: List[Dict[str, Any]], region: str, top_n: int) -> None:
    for it in _select_top(items, top_n):
        ...
        kind=(it.get("media_type") or "").lower(); tid=it.get("tmdb_id")
    ...
    for it in _select_top(items, top_n):
        ...
        try:
            ...
            for k,v in det.items():
                if v and it.get(k) in (None,[], "", {}): it[k]=v
        except Exception: pass
        try:
...
def main() -> None:
    t0=time.time()
    run_self_check()
    OUT_ROOT.mkdir(parents=True, exist_ok=True); CACHE_ROOT.mkdir(parents=True, exist_ok=True)
    run_dir=_build_run_dir(); log_path=run_dir/"runner.log"; diag_path=run_dir/"diag.json"
    exports_dir=run_dir/"exports"; exports_dir.mkdir(parents=True, exist_ok=True)
    log_lines: List[str]=[]
    def _log(s:str)->None:
    ...
    try:
        ...
        _log(f"[exclusions] strict filter: removed={excl_info['excluded_count']} (ratings_ids~{excl_info['ratings_rows']}, public_ids_add={excl_info['public_ids']})")
    except Exception as ex:
    ...
    try:
        ...
        profile_t={"rows": int(model.get("meta",{}).get("count",0)), "global_avg": model.get("meta",{}).get("global_avg"), "path": model_path}
    except Exception as ex:
