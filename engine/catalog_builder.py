...
OMDB = "https://www.omdbapi.com/"  # ← HTTPS (fix)
    ...
    if cache_file.exists():
        ...
        if age < 3600 * ttl_hours:
            try:
                ...
                pass  # fall through to refetch if cache is corrupt
    try:
        r = requests.get(url, headers=headers, timeout=30)
    except Exception as e:
        print(f"[http] GET failed: {e} for {url.split('?')[0]}")
        return None

    if r.status_code != 200:
        print(f"[http] {r.status_code} for {url.split('?')[0]} (no body cached)")
        return None

    try:
        data = r.json()
    except Exception:
        print(f"[http] non-JSON for {url.split('?')[0]}")
        return None
...
def _tmdb_headers():
    ...
    # Accept both v4 Bearer or legacy ?api_key param (we use Bearer here)
    return {"Authorization": f"Bearer {key}"} if key else {}

# Map TMDB provider names → our slugs
PROV_MAP = {
...
# One-line log on first successful enrich so you can see it's working
_OMDB_FIRST_OK = False
...
def _omdb_enrich(title: str, year: int, media_type: str, imdb_id: str = ""):
    ...
    data = _cached_get_json(url, ttl_hours=168)  # 7 days cache
    if not data or data.get("Response") == "False":
        if data and data.get("Error"):
            print(f"[omdb] miss: {title} ({year}) → {data.get('Error')}")
        return {"critic":0.0,"audience":0.0,"language_primary":"","imdb_id":imdb_id or "","genres":[]}
    ...
    global _OMDB_FIRST_OK
    if not _OMDB_FIRST_OK:
        rt_disp = [r.get('Value') for r in (data.get('Ratings') or []) if r.get('Source')=='Rotten Tomatoes']
        print(f"[omdb] ok: {data.get('Title','?')} ({data.get('Year','?')}) IMDb={data.get('imdbRating','?')} RT={rt_disp}")
        _OMDB_FIRST_OK = True
...
def build_catalog():
    ...
    # English originals only (based on OMDb primary language string)
