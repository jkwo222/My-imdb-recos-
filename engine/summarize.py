...
# NEW: Early provider fetch (pass 1 as well)
...
def _providers_for_item(it: Dict[str, Any], allowed: Iterable[str]) -> List[str]:
    ...
    # Fallback to network names for TV if configured
...
def _ensure_providers(it: Dict[str, Any], region: str) -> None:
    """Fetch providers on-demand if missing (uses TMDB)."""
    ...
    try:
        ...
        # Swallow and leave providers empty if TMDB is flaky
        pass
...
def _recency_label(it: Dict[str, Any]) -> Optional[str]:
    ...
    if mt == "movie":
        ...
        if d is not None and d <= REC_MOVIE_WINDOW and LAB_NEW_MOVIE:
            return "New Movie"
    if mt == "tv":
        seasons = int(it.get("number_of_seasons") or 0)
        df = _days_since(_parse_ymd(it.get("first_air_date")))
        if df is not None and df <= REC_TV_FIRST_WINDOW and LAB_NEW_SERIES:
            return "New Series"
        if seasons >= 2:
            dl = _days_since(_parse_ymd(it.get("last_air_date")))
            if dl is not None and dl <= REC_TV_LAST_WINDOW and LAB_NEW_SEASON:
                return "New Season"
    return None
...
def _fmt_meta_line(it: Dict[str, Any], providers: List[str]) -> str:
    try:
        match = int(round(float(it.get("score", 0) or 0)))
    except Exception:
        match = None
    ...
    if isinstance(match, (int, float)):
        parts.append(f"Match {int(match)}")
...
def render_email(
    ...
    *,
    ...
    def _eligibility_reason(it: Dict[str, Any], min_score: int, allow_rotate: bool) -> Optional[str]:
        ...
        if float(it.get("score", 0) or 0) < min_score:
            return "score_below_cutoff"
        if EMAIL_EXCLUDE_ANIME and _is_anime_like(it):
            return "anime_excluded"
    ...
    def _collect(min_score: int, *, allow_rotate: bool, try_fetch_providers: bool):
        ...
        for it in sorted(
            ranked_items,
            key=lambda x: float(x.get("score", x.get("tmdb_vote", 0.0)) or 0.0),
            reverse=True,
        ):
            # If we don't have providers, optionally fetch them *before* eligibility check
            ...
            # If the only reason is missing providers and we haven't tried a fetch yet,
            # do a one-shot fetch and re-evaluate (extra safety even when try_fetch_providers=False)
            ...
            if (it.get("media_type") or "").lower() == "movie":
                if m_cnt < EMAIL_TOP_MOVIES:
                    movies.append(block_text)
                    m_cnt += 1
                    breakdown["selected_movies"] += 1
                    if key:
                        chosen_keys.append(key)
            else:
                if s_cnt < EMAIL_TOP_TV:
                    shows.append(block_text)
                    s_cnt += 1
                    breakdown["selected_tv"] += 1
                    if key:
                        chosen_keys.append(key)
    ...
    # Pass 1 (now optionally fetch providers early)
    movies, shows, keys = _collect(
        EMAIL_SCORE_MIN,
        allow_rotate=True,
        try_fetch_providers=EMAIL_EARLY_FETCH_PROVIDERS,
    )

    # Pass 2 (backfill) — keep provider fetch here as well
    used_backfill = False
    ...
    if EMAIL_INCLUDE_TELEMETRY:
        ...
        if diag:
            ...
            if c:
                # clearer labels (avoid "discovered" confusion)
