# File: engine/ratings_ingest.py
import os, io, csv, time, requests, re
...
def _map_type(raw: str) -> str:
    ...
    if s in {"feature", "movie", "video"}: return "movie"
    if s in {"tvseries", "tv series"}: return "tvSeries"
    if s in {"tvminiseries", "tv mini-series", "tv mini series"}: return "tvMiniSeries"
    if s in {"tvmovie", "tv movie"}: return "tvMovie"
    if s in {"tvspecial", "tv special"}: return "tvSpecial"
    if s in {"short"}: return "movie"
    if s in {"videogame", "video game"}: return "game"
    if raw in {"tvSeries","tvMiniSeries","tvMovie","tvSpecial","movie","game"}: return raw
    return "movie"
...
def _get_any(row: Dict[str,str], *names: str) -> str:
    ...
    for n in names:
        ...
        if key is not None: return row[key]
    return ""
...
def _ratings_url_from_env() -> str:
    ...
    if ratings_url: return ratings_url
    if user_id: return f"https://www.imdb.com/user/{user_id}/ratings?sort=ratings_date:desc&mode=detail"
    return ""
...
def _scrape_page(url: str):
    ...
    for b in blocks:
        ...
        if ytag:
            my = re.search(r"(\d{4})", ytag.get_text()); y = int(my.group(1)) if my else 0
...
def load_user_ratings_combined():
    ...
    if not rows_csv:
        try: rows_csv = load_from_csv_url()
        except Exception as e: rprint(f"[yellow][IMDb CSV] URL failed: {e}[/yellow]")
    ...
    if start_url:
        ...
        while url and pages < 50:
            ...
            consecutive_known_pages = 0 if new_on_page else (consecutive_known_pages + 1)
