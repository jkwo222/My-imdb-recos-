...
def update_from_ratings(rows):
    # Nudge critic vs audience slightly based on your recent ratings signal
    pos = sum(1 for r in rows if float(r.get("your_rating",0)) >= 8)
    neg = sum(1 for r in rows if 0 < float(r.get("your_rating",0)) <= 5)
    total = len(rows) or 1
    delta = (pos - neg)/total
    w = load_weights()
    w["critic_weight"] = float(min(0.7, max(0.3, w.get("critic_weight",0.52) + 0.04*delta)))
